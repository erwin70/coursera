...
function contact (){
    ...
    if (isset($_POST['submit'])){
        if (isset($_POST['jeBevesteging'])){
            $headers = 'From: Website <email@email.nl>'."\n";
            $headers .= 'Reply-To: '.$_POST['jeNaam'].' <'.$_POST['jeEmail'].'>'."\n";
            $headers .= 'X-Mailer: PHP/' . phpversion()."\n";;
            $headers .= 'Return-Path: Mail-Error <email@email.nl>'."\n";
            $headers .= 'MIME-Version: 1.0'."\n";
            $headers .= 'Content-Transfer-Encoding: 8bit'."\n";
            $headers .= 'Bcc: '.$_POST['jeEmail']."\n";
            $headers .= 'Content-type: text/html; charset=iso-8859-1'."\n";
            
            if (mail('Davey Raaijmakers <email@email.nl>', 'Vraag van, '.$_POST['jeNaam'], $_POST['jeVraag'], $headers)){
